...
class Rama::ResourcesController < ApplicationController
  ...
  def update
    if @resource.update(resource_params)
      respond_to do |format|
        ...
        format.turbo_stream do
          render turbo_stream: [
            turbo_stream.replace("resource_#{@resource.id}", partial: 'resource_row',
                                                             locals: { resource: @resource },),
            turbo_stream.update('modal', ''),
  ...
  def build_filtered_scope
    ...
    # Apply search
    scope = apply_search(scope, params[:search]) if params[:search].present?
  ...
  def apply_search(scope, query)
    ...
    conditions = searchable_fields.map { |field_name, _|
