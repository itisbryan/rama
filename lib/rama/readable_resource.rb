  ...
  class Resource
    ...
    class << self
      ...
      def build_scope(params = {})
        ...
        if performance_config[:includes].present?
          scope = scope.includes(*performance_config[:includes])
        end
      ...
      def format_field_value(record, field_name)
        ...
        when :boolean
          value = record.public_send(field_name)
          field_config[:options][:true_label] && value ? field_config[:options][:true_label] : 
          field_config[:options][:false_label] && !value ? field_config[:options][:false_label] :
          value.to_s.humanize
        else
          record.public_send(field_name)
        end
