...
module Rama
  class SearchEngine
    ...
    def trigram_search
      ...
      order_clause = searchable_columns.map do |column|
        "similarity(#{column}, ?)"
      end.join(', ')
    ...
    def elasticsearch_search
      ...
      model_class.search(
        ...
        highlight: {
          fields: searchable_columns.each_with_object({}) { |col, hash| hash[col] = {} }
        }
      )
    end
    ...
    def detect_searchable_columns
      ...
      model_class.columns.each do |column|
        if [:string, :text].include?(column.type)
          columns << "#{model_class.table_name}.#{column.name}"
        end
      end
    ...
    def suggestion_columns
      ...
      if model_class.column_names.include?('name')
        ...
        base_columns << searchable_columns.first&.split('.')&.last&.to_sym
      end
    ...
    def format_suggestion(values)
      ...
      {
        id: values.first,
        text: values[1..-1].join(' - ')
      }
    end
    
    def detect_search_vector_column
      # Look for common search vector column names
      %w[search_vector search_data searchable_text].find do |col_name|
